...
#         records = []
#         summary = {'total_users': 0, 'total_present': 0, 'in_progress': 0, 'total_absent': 0}
#         pending_checkouts = []
...
# @scheduler.task('cron', id='auto_checkout_job', hour=23, minute=59, second=59)
# def scheduled_auto_checkout():
#     """Automatically checkout users at 11:59:59 PM and set auto_checkout=1"""
#     conn = get_db_connection()
#     try:
#         with conn.cursor() as cursor:
#             today = date.today()
#             checkout_time = datetime.combine(today, time(23, 59, 59))
...
#                 cursor.execute("SELECT id FROM t_attendance_hr WHERE user_id=%s AND DATE(checkin_time) = CURDATE()", (user_id,))
#                 if cursor.fetchone():
...
#                     )
#                     if location_name:
#                         msg = f"Checked in successfully from {location_name}."
#                     else:
#                         msg = "Checked in successfully."
                        
#             elif action == 'checkout':
#                 # Find open attendance record to check out
#                 cursor.execute(
#                     "SELECT id, checkin_time FROM t_attendance_hr WHERE user_id=%s AND DATE(checkin_time) = CURDATE() AND checkout_time IS NULL",
#                     (user_id,)
#                 )
#                 attendance = cursor.fetchone()
#                 if attendance:
#                     # Convert checkout location data
#                     check_out_latitude = float(latitude) if latitude else None
#                     check_out_longitude = float(longitude) if longitude else None
                    
#                     # Update with checkout time and checkout location (INCLUDING check_out_location_name)
#                     cursor.execute(
...
#                     )
#                     if location_name:
#                         msg = f"Checked out successfully from {location_name}."
#                     else:
#                         msg = "Checked out successfully."
#                 else:
#                     msg = "You must check in first or have already checked out."

#     # Get user's attendance history (updated to include check_out_location_name)
#     with conn.cursor() as cursor:
#         cursor.execute("""
#         SELECT
#             u.firstname,
...
#     """Generate CSV export of monthly report"""
#     import calendar
...
#     # Write updated column headers (removed: Average Hours/Day, Attendance %, First Check-in, Last Check-out)
#     # Added: Total Working Days, Week Offs after Employee Name
#     writer.writerow([
#         'Employee Name', 'Total Working Days', 'Week Offs', 'Days Present', 'Days Completed', 'Total Hours', 'Incomplete Days'
#     ])
...
#         # Title
#         title = Paragraph(f"Medscred HRMS - Monthly Attendance Report<br/>{selected_month}", title_style)
...
#         FROM t_attendance_hr a
#         JOIN t_users_hr u ON a.user_id = u.id
#         WHERE TIME(a.checkin_time) > '09:30:00'
...
#         FROM t_attendance_hr a
#         JOIN t_users_hr u ON a.user_id = u.id
#         WHERE TIME(a.checkout_time) < '17:30:00'
        ...
        with conn.cursor() as cursor:
            if action == 'checkin':
                ...
                if attendance:
                    ...
                    # Update with checkout time and checkout location
                    cursor.execute(
                        "UPDATE t_attendance_hr SET checkout_time=%s, work_duration=TIMEDIFF(%s, checkin_time), check_out_longitude=%s, check_out_latitude=%s, check_out_location_name=%s WHERE id=%s",
                        (now, now, check_out_longitude, check_out_latitude, location_name, attendance['id'])
                    )
